...
const REMOTE_SERVICE = '/dry/service-rules-usage-blbc';
...
void initOptions(){
  ...
  .then((datasource) => fetchOptions({"opt":"formats","ds":datasource}, formats, "")
...
void datasourceOnChange(){
  fetchOptions({"opt":"formats","ds":datasources.value}, formats, "")
...
void fetchCustomization(){
  loading(true);
  var url = getBaseUrl(REMOTE_SERVICE)+getParamsString({"opt":"customization","tp":tps.value,"ds":datasources.value,"fmt":formats.value,"cvt":'Customization',"seg":'Customization',"snum":'Customization',"case":'*'});
  var request = HttpRequest.getString(url).then((jstr){
